...
class Game{
  constructor(ctx, gv, retry = false){
    this.gv = gv;
    this.retry = retry;
    this.bombs = 3;
    this.score = 0;
    ...
    setInterval(() => {
      if(this.asteroids[0].name ==='tutorial'){
        this.asteroids[0].textColor === 'orange' ? this.asteroids[0].textColor = 'white' : this.asteroids[0].textColor = 'orange';
      }
    }, 1000);
