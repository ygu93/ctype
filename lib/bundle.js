	...
	var GameView = function () {
	  ...
	  _createClass(GameView, [{
	    ...
	    value: function restart() {
	      ...
	      this.game = new _game2.default(this.ctx, this, true);
	    ...
	    value: function start() {
	      ...
	      var gameStart = setInterval(function () {
	        if (_this3.game.ship === null) {
	          clearInterval(gameStart);
	          _this3.game.gameOver();
	          _this3.game = new _game2.default(_this3.ctx, _this3);
	        }
	        _this3.game.draw(_this3.ctx);
	        _this3.game.step();
	      }, 20);
	    }
	  }, {
	    ...
	    value: function bindKeyHandlers() {
	      ...
	      alphabet.forEach(function (letter) {
	        return key(letter, function () {
	          return _this4.game.getInput(letter);
	        });
	      });
	      key('enter', function () {
	        return _this4.game.useBomb();
	      });
	    }
	  }, {
	    key: 'bindPauseButton',
	    value: function bindPauseButton(play) {
	      var _this5 = this;
	      ...
	      canvas.addEventListener("click", function (e) {
	        ...
	        if (clickedX >= 430 && clickedY >= 560 && _this5.running) {
	          _this5.running = false;
	          clearInterval(play);
	        } else if (clickedX >= 430 && clickedY >= 560 && !_this5.running) {
	          (function () {
	            _this5.running = true;
	            var gameStart = setInterval(function () {
	              if (_this5.game.ship === null) {
	                clearInterval(gameStart);
	                _this5.game.gameOver();
	                _this5.game = new _game2.default(_this5.ctx, _this5);
	              }
	              _this5.game.draw(_this5.ctx);
	              _this5.game.step();
	            }, 20);
	          })();
	        }
	    ...
	    value: function bindVolumeButton() {
	      ...
	      canvas.addEventListener("click", function (e) {
	        var clickedX = e.pageX - canvas.offsetLeft;
	        var clickedY = e.pageY - canvas.offsetTop;
	        if (clickedX <= 50 && clickedY >= 560 && _this6.volume) {
	          document.getElementById('bg-music').pause();
	          _this6.volume = !_this6.volume;
	        } else if (clickedX <= 50 && clickedY >= 560 && !_this6.volume) {
	          document.getElementById('bg-music').play();
	          _this6.volume = !_this6.volume;
	        }
	      });
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Game = function () {
	  function Game(ctx, gv) {
	    ...
	    this.ctx = ctx;
	    this.wave = 0;
	    this.tutorialMsg = 'rgba(255,255,255, 0.5)';
	    this.astVel = 1000;
	    ...
	    var tutorialFlash = setInterval(function () {
	      if (_this.wave === 0) {
	        _this.asteroids[0].textColor === 'orange' ? _this.asteroids[0].textColor = 'white' : _this.asteroids[0].textColor = 'orange';
	      } else {
	        clearInterval(tutorialFlash);
	      }
	    }, 1000);
	
	    // let tutorialMessage = setInterval(()=>{
	    //     this.tutorialMsg = `rgba(255, 255, 255, ${this.opacity})`;
	    //     this.opacity += 0.1;
	    //     if(this.opacity >= 1){
	    //       this.opacity = 0.5;
	    //     }
	    //     // this.tutorialMsg === 'rgba(255,255,255, 0.8)' ? this.tutorialMsg = 'rgba(255,255,255, 1)' : this.tutorialMsg = 'rgba(255,255,255, 0.8)';
	    //     if(this.wave > 0){
	    //       clearInterval(tutorialMessage);
	    //     }
	    //   }, 200);
	  }
	  ...
	  _createClass(Game, [{
	    ...
	    key: 'generateTutorial',
	    value: function generateTutorial() {
	      var _this2 = this;
	
	      var tutorial = new _asteroid2.default({ pos: this.randomPosition(), name: 'tutorial' }, this, this.astVel);
	      this.asteroids.push(tutorial);
	      var tutorialMessage = setInterval(function () {
	        _this2.tutorialMsg = 'rgba(255, 255, 255, ' + _this2.opacity + ')';
	        _this2.opacity += 0.1;
	        if (_this2.opacity >= 1) {
	          _this2.opacity = 0.5;
	        }
	    ...
	    value: function nextLevel() {
	      ...
	      var nextWave = setInterval(function () {
	        _this3.generateAsteroid();
	        if (_this3.asteroids.length + _this3.destroyedAsteroids.length === Game.NUM_ASTEROIDS) {
	      ...
	      if (this.bombs > 0) {
	        ...
	        this.asteroids.forEach(function (asteroid) {
	          var explosion = setInterval(function () {
	            return _this4.drawExplosion(_this4.ctx, asteroid);
	          }, 10);
	          _this4.playExplosion();
	          setTimeout(function () {
	            return clearInterval(explosion);
	          }, 300);
	          _this4.destroyedAsteroids.push(asteroid);
	        });
	        this.asteroids = [];
	        this.currentWordIndex = "";
	        this.bombs -= 1;
	    ...
	    value: function drawStats() {
	      ...
	      this.ctx.fillText('Wave ' + this.wave + ' Score: ' + this.score + ' EMPs: ' + this.bombs, 90, 590);
	    }
	  }, {
	    key: 'drawTutorialMsg',
	    value: function drawTutorialMsg() {
	      this.ctx.fillStyle = this.tutorialMsg;
	      this.ctx.font = "900 18px Orbitron";
	      this.ctx.fillText("Type tutorial to destroy the asteroid!", 60, 450);
	    }
	  }, {
	    key: 'draw',
