	  ...
	  _createClass(GameView, [{
	    ...
	    value: function start() {
	      ...
	      canvas.addEventListener("click", function (e) {
	        ...
	        if (clickedX >= 430 && clickedY >= 560 && _this.running) {
	          _this.running = false;
	          clearInterval(gameStart);
	        } else if (clickedX >= 430 && clickedY >= 560 && !_this.running) {
	          _this.running = true;
	    ...
	    value: function bindKeyHandlers() {
	      ...
	      alphabet.forEach(function (letter) {
	        return key(letter, function () {
	          return _this2.game.getInput(letter);
	        });
	      });
	      key('enter', function () {
	        return _this2.game.useBomb();
	      });
	    }
	  }, {
	    ...
	    value: function bindVolumeButton() {
	      ...
	      canvas.addEventListener("click", function (e) {
	        var clickedX = e.pageX - canvas.offsetLeft;
	        var clickedY = e.pageY - canvas.offsetTop;
	        if (clickedX <= 50 && clickedY >= 560 && _this3.volume) {
	          document.getElementById('bg-music').pause();
	          _this3.volume = !_this3.volume;
	        } else if (clickedX <= 50 && clickedY >= 560 && !_this3.volume) {
	          document.getElementById('bg-music').play();
	          _this3.volume = !_this3.volume;
	        }
	      });
	    ...
	    key: 'bindPauseButton',
	    value: function bindPauseButton() {
	      var _this4 = this;
	      ...
	      canvas.addEventListener("click", function (e) {
	        var clickedX = e.pageX - canvas.offsetLeft;
	        var clickedY = e.pageY - canvas.offsetTop;
	        if (clickedX <= 430 && clickedY >= 560 && _this4.running) {
	          // clearinterval
	          _this4.running = !_this4.running;
	        } else if (clickedX <= 430 && clickedY >= 560 && !_this4.running) {
	          // setInterval
	          _this4.running = !_this4.running;
	        }
	      });
